...
dat_child <- dat %>% select(1:3) %>% rename_all(~stringr::str_replace(.,"^child_","")) %>%  mutate(age="child") %>% 
  drop_na(change) %>% mutate(change=as.factor(change)) #%>% mutate(change=fct_rev(change)) #%>% filter(wave_1_titre>1)
...
run_model <- function(data.list){
  ...
  model <- function(){
    ...
    for(i in 1:n)  
    {
      ...
      S[i] <- a + c * exp(-exp(-b*(t[i]-tm)))
      
      Y[i]~dnorm(S[i],lambda3)
    ...
    lambda3~dgamma(0.0001,0.0001)
  }
  
  jags(model.file=model,
       data=data.list,
       parameters.to.save=c("S","b","a","c","reduction","exp_tm","tm","thresh_50","thresh_95","min_Y","max_Y"),
       n.iter=2.5e3,
       n.chains = 2,
       n.burnin = 2.5e2)
...
mother_res <- run_model(data.list = list(n=nrow(dat_mother)+length(pred_t),
                                         Y=c(dat_mother$wave_2_titre,rep(NA,length(pred_t))),
...
child_res <-  run_model(data.list = list(n=nrow(dat_child)+length(pred_t),
                                         Y=c(wave_2_titre,rep(NA,length(pred_t))),
